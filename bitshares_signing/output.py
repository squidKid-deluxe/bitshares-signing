...
def graphenize_cancel(rpc, cancel_edicts, fees, order, account_name, account_id, tx_operations):
    """
    ...
    for edict in cancel_edicts:
        ...
        for order_id in edict["ids"]:
            ...
            operation = [
                ...
                {"fee": fee, "fee_paying_account": account_id, "order": order_id, "extensions": []},
            ]
...
def graphenize_limit_orders(
    ...
    for idx, edict in enumerate(create_edicts):
        ...
        operation = [
            1,
            {"fee": fee, "seller": account_id, "amount_to_sell": amount_to_sell, "min_to_receive": min_to_receive, "expiration": op_expiration, "fill_or_kill": KILL_OR_FILL, "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_issue(
    ...
    for idx, issue in enumerate(issue_edicts):
        ...
        operation = [
            14,
            {"fee": fee, "issuer": account_id, "asset_to_issue": amount_dict, "issue_to_account": issue["account_id"], "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_reserve(
    ...
    for idx, reserve in enumerate(reserve_edicts):
        ...
        operation = [
            15,
            {"fee": fee, "payer": account_id, "amount_to_reserve": amount_dict, "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_transfer(
    ...
    for idx, transfer in enumerate(transfer_edicts):
        ...
        operation = [
            0,
            {"fee": fee, "from": account_id, "to": transfer["account_id"], "amount": amount_dict, "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_login(login_edicts, fees, account_id, tx_operations):
    ...
    if login_edicts:
        ...
        operation = [
            2,
            {"fee": fee, "fee_paying_account": account_id, "order": "1.7.0", "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_swaps(swap_edicts, fees, asset, currency, account_id, tx_operations):
    ...
    for edict in swap_edicts:
        ...
        operation = [
            63,
            {"fee": fee, "account": account_id, "pool": pool, "amount_to_sell": amount_to_sell, "min_to_receive": min_to_receive, "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_pool_creation(
    ...
    for pool_create_edict in pool_create_edicts:
        ...
        operation = [
            59,
            {"fee": {"amount": 5000000, "asset_id": "1.3.0"}, "account": account_id, "asset_a": asset_id, "asset_b": currency_id, "share_asset": pool_create_edict["share_asset"], "taker_fee_percent": int(pool_create_edict["taker_fee_percent"] * 100), "withdrawal_fee_percent": int(pool_create_edict["withdrawal_fee_percent"] * 100)},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_call(
    ...
    for call in call_edicts:
        # convert to graphene amount, asset_id type
        delta_debt = {"amount": int(call["debt_delta"] * 10**asset_precision), "asset_id": asset_id}
        delta_collateral = {"amount": int(call["collateral_delta"] * 10**currency_precision), "asset_id": currency_id}
        ...
        operation = [
            3,
            {"fee": fee, "funding_account": account_id, "delta_collateral": delta_collateral, "delta_debt": delta_debt, "extensions": {"target_collateral_ratio": int(1000 * call["tcr"])}},
        ]
...
def graphenize_pool_deposit(
    ...
    for pool_deposit in pool_deposit_edicts:
        ...
        operation = [
            61,
            {"fee": fee, "account": account_id, "pool": pool_deposit["pool"], "amount_a": {"amount": int(
                                        pool_deposit["amount_a"] * 10**asset_precision
                                    ), "asset_id": asset_id}, "amount_b": {"amount": int(
                                        pool_deposit["amount_b"]
                                        * 10**currency_precision
                                    ), "asset_id": currency_id}, "extensions": []},
        ]
        # add this op to our list of ops
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_fee_pool(
    ...
    for fee_pool in fee_pool_edicts:
        ...
        operation = [
            47,
            {"fee": {"amount": fees["fee_pool"], "asset_id": "1.3.0"}, "issuer": fee_pool["issuer"], "asset_id": fee_pool["asset_id"], "amount_to_claim": {"amount": fee_pool["amount_to_claim"], "asset_id": "1.3.0"}, "extensions": []},
        ]
        # add this op to our list of ops
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_asset_create(
    ...
    for asset_create_edict in asset_create_edicts:
        ...
        operation = [
            10,
            {"fee": {"amount": 96700000, "asset_id": "1.3.0"}, "issuer": asset_create_edict["issuer"], "symbol": asset_create_edict["symbol"], "precision": asset_create_edict["precision"], "common_options": {"max_supply": common_options["max_supply"], "market_fee_percent": common_options["market_fee_percent"], "max_market_fee": common_options["max_market_fee"], "issuer_permissions": common_options["issuer_permissions"], "flags": common_options["flags"], "core_exchange_rate": common_options["core_exchange_rate"], "whitelist_authorities": common_options["whitelist_authorities"], "blacklist_authorities": common_options["blacklist_authorities"], "whitelist_markets": common_options["whitelist_markets"], "blacklist_markets": common_options["blacklist_markets"], "description": common_options["description"], "extensions": []}, "is_prediction_market": asset_create_edict["is_prediction_market"], "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_publish(publish_edicts, fees, rpc, account_id, tx_operations):
    ...
    for edict in publish_edicts:
        ...
        feed = {"settlement_price": settlement_price, "maintenance_collateral_ratio": edict["MCR"], "maximum_short_squeeze_ratio": edict["MSSR"], "core_exchange_rate": core_exchange_rate}
        operation = [
            19,  # nineteen means "Publish Price Feed"
            {"fee": fee, "publisher": account_id, "asset_id": asset_id, "feed": feed, "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def graphenize_add_producer(producer_edicts, fees, account_id, tx_operations):
    ...
    for edict in producer_edicts:
        ...
        operation = [
            ...
            {"fee": fee, "issuer": account_id, "asset_to_update": edict["asset_id"], "new_feed_producers": edict["producer_ids"], "extensions": []},
        ]
        tx_operations.append(operation)
    return tx_operations
...
def build_transaction(rpc, order):
    ...
    graphenize_cancel(rpc, edict_types["cancel"], fees, order, account_name, account_id, tx_operations)
